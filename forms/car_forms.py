...
class CarForm(FlaskForm):
    ...
    registration_card = FileField(
        "Registration Card",
        validators=[FileAllowed(["jpg", "jpeg", "png"])],
    )
    kilometers = IntegerField("Kilometers Driven", validators=[DataRequired()])  # Add this line
    no_of_owners = IntegerField("Number of Previous Owners", validators=[DataRequired(), NumberRange(min=1, max=10)])  # Add this line - and max
    state = SelectField("State", choices=[], validators=[DataRequired()])  # New field
    district = SelectField("District", choices=[], validators=[DataRequired()])  # New field, ADD DataRequired
    ...
    engine_capacity = FloatField("Engine Capacity", validators=[Optional()])
